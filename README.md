# Cat Mayhem
##### you meet a cat yearning for the outdoors - will you join him or not?

---

### Contents

🐱 [Information](#information)


🐱 [Gameplay](#gameplay)


🐱 [Features](#features)


🐱 [Ai Use](#ai-use)


🐱 [Learning](#learning)


🐱 [Credits](#credits)


🐱 [Screenshots](#screenshots)


## Information

---

🐱  **Github Repo:**  https://github.com/jyoshitanair/text-game

🐱  **Itch.io:** https://jyoshitanair.itch.io/cat-mayhem

🐱  **Created by:** jyoshitanair (github)

🐱 **Made With:** Godot Version 4.6.1 Stable gd script and itch.io!


## Description

---

You are a cat that just moved in to their new purrever home! (yes the puns are necessary).

You find out that you have a kitty roomate who is absolutely obssessed with the ideas of going outside.
In this game you will have a variety of different activities to do with your kitty friend and try to survive!
It's very inspired by those old terminal games - but made better? and it has ASCII code art!
It is also a visual novel!


## Gameplay

---

🐱 you can move through the options with the **down arrow**

🐱 you can click on an option with the **enter key**

🐱 you can pause with the **p key**

🐱 the only time you need to click or type is in the **settings menu!**


## Features

---

🐱 I made a **template called dialoge** to reuse for all the text to save time :D

🐱 There's an awesome looking **cover screen** if I do say so myself :3 

🐱 A **Settings menu** that let's you pick *your name, background music, the volume, and see your past endings!*

🐱 A **How to play Menu** incase you don't want to go alll the way back to this read me

🐱 An amazing **read me** :D if I do say so myself!

🐱 A "fake" **loading screen** (bascially it doesn't NEED to load but I kept it as a break between sections) that has 7 randomly generated phrases on the top

🐱 During the dialogue sometimes the **ASCII art changes** (woahhh!)

🐱 **11 different endings?!** ( ˶o˶˶o˶) that are good and bad

🐱 **22 different paths** (scenes) that you can go to! 


## Ai-Use

🐱 I only used Gemini for very very minimal debugging and all code is written by me!

🐱 No media is AI generated ofc!


## Learning


🐱 Honestly a lot of this was old news to me but I DID learn how to make a triangle shape in godot! (super impressive fr)

🐱 I definetly got a lot faster at this too! 

🐱 My first time making a loading screen? I think it's pretty cool might have to do it again (UwU)

🐱 I learned how to finally get arounf ASCII font restrictions using DejaVu Sans as a back up! I didn't know you could do that and it's so helpful!!


## Credits

---


1) Fonts

	🐱 CS Morcant Mono Font by **craftsupplyco** on fontspace : https://www.fontspace.com/cs-morcant-mono-font-f126413 - craftsupplyco CS Morcant Mono Font
	
	🐱 Code Rider by **Denu Studios** on fontspace : https://www.fontspace.com/code-rider-font-f137719

	🐱 DejaVu Sans by **FontSquirrel** : https://www.fontsquirrel.com/fonts/dejavu-sans

	🐱 All the ASCII art was sourced from **https://emojicombos.com/** and a few were edited by me after!


3) Music

	🐱 You Call It Chaos - I Call It Grace by **Outlaw Creative** on pixabay : https://pixabay.com/music/funk-you-call-it-chaos-i-call-it-grace-392666/
	
	🐱 Cat Nap (lofi hiphop beats) by **PurrpleCat** on pixabay : https://pixabay.com/music/beats-cat-nap-lofi-hiphop-beats-199252/
	
	🐱 Dramatic - Dramatic Music by **The_Mountain** on pixabay : https://pixabay.com/music/epic-classical-dramatic-dramatic-music-508006/
	
	🐱 Rock - Rock Music by **alex-morgan** on pixabay : https://pixabay.com/music/rock-rock-rock-music-545492/


## Screenshots

---

#### Cover
<img width="1127" height="611" alt="cover" src="https://github.com/user-attachments/assets/a7799248-56f8-4e16-833a-b5250a19e183" />

#### Dialoge
<img width="760" height="367" alt="dialog-cat" src="https://github.com/user-attachments/assets/e1a0d99b-5cc2-4572-a022-4ab7249ed6f8" />

#### Menu
<img width="707" height="407" alt="menu-cat" src="https://github.com/user-attachments/assets/75c0e217-244c-4049-82dd-73776f2d5fee" />

#### Settings
<img width="752" height="426" alt="settings-cat" src="https://github.com/user-attachments/assets/93515bc8-dd21-4d58-9047-2d4d2c48f223" />

#### Loading

<img width="702" height="410" alt="loading-cat" src="https://github.com/user-attachments/assets/b29e490a-18ea-4502-834e-104ba0323996" />

#### Bonus Images
<img width="267" height="437" alt="angry-cat" src="https://github.com/user-attachments/assets/a753aeaf-7d80-4863-9e26-db49c854cd1f" />

<img width="340" height="395" alt="wierdcat" src="https://github.com/user-attachments/assets/2c943a54-3f6d-49c7-859f-94f5c41365e7" />

<img width="266" height="436" alt="sad-cat" src="https://github.com/user-attachments/assets/5dcf06dc-700e-4fe1-9009-4341dc7eaaa5" />

<img width="312" height="450" alt="shocked-cat" src="https://github.com/user-attachments/assets/759cbdb0-a8ec-4862-a0a1-424afb373ede" />

<img width="262" height="427" alt="cat-normal" src="https://github.com/user-attachments/assets/8588480e-ed37-407b-ba08-82e9e01c91b4" />

<img width="401" height="322" alt="cat-sleep" src="https://github.com/user-attachments/assets/7f2a259f-8f79-4ea1-aa1b-4420485884ef" />


*Made with 💖 by jyoshita!*
